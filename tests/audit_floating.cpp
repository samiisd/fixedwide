...
int main(){int f=0;auto chk=[&](bool ok,const char*n){std::cout<<(ok?"PASS ":"FAIL ")<<n<<'\n';if(!ok)++f;};
 ...
 long double ld=1.0L+std::ldexp(1.0L,-60);auto d=from_float<Fixed128<18>>(ld,Rounding::nearest_even);wide::int128 expected(1'000'000'000'000'000'001LL);if(d)std::cout<<"long double raw low="<<d->raw().low<<" high="<<d->raw().high<<'\n';chk(d&&d->raw()==expected,"long double conversion keeps precision advertised by template");
 std::cout<<"failures="<<f<<'\n';return f?1:0;}

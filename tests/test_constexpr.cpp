...
unsigned error_code(fixedwide::ArithmeticError e) {
...
void agree(const char* op, const Ref& reference, const Kern& kernel, long long a, long long b, unsigned digits,
           Rounding mode) {
    ...
    if (!same_shape) {
        std::fprintf(stderr, "%s a=%lld b=%lld digits=%u mode=%u: ref %s, kernel %s\n", op, a, b, digits,
                     static_cast<unsigned>(mode), reference ? "ok" : "error", kernel ? "ok" : "error");
    }
    CHECK(same_shape);
    if (reference.has_value()) {
        ...
        if (reference.error() != kernel.error()) {
            std::fprintf(stderr, "%s a=%lld b=%lld digits=%u mode=%u: ref err=%u kernel err=%u\n", op, a, b, digits,
                         static_cast<unsigned>(mode), error_code(reference.error()), error_code(kernel.error()));
        }
...
void compare_one(Fixed a, Fixed b, Fixed c, Rounding mode) {
    ...
    agree("mul_div", ce::mul_div<Bits, D>(a.raw(), b.raw(), c.raw(), mode), fixedwide::mul_div(a, b, c, mode), ia, ib,
          0, mode);
    for (unsigned digits = 0; digits <= D; ++digits) {
        agree("quantize", ce::quantize<Bits, D>(a.raw(), digits, mode), fixedwide::quantize(a, digits, mode), ia, ib,
              digits, mode);
    }
    ...
    for (auto x : values) {
        for (auto y : values) {
            for (auto mode : all_modes) {
                compare_one<Bits, D, Fixed>(Fixed::from_raw(x), Fixed::from_raw(y), Fixed::from_raw(y), mode);
            }
        }
    }
}
...
int main() {
    ...
    static_assert(fixedwide::mul(T::from_raw(2500000000000), T::from_raw(4000000000000))->raw() == 10000000000000);
    static_assert(fixedwide::div(T::from_raw(1000000000000), T::from_raw(3000000000000))->raw() == 333333333333);
    static_assert(!fixedwide::div(T::from_raw(1), T::from_raw(0)).has_value());
    static_assert(!fixedwide::div(T::from_raw(1000000000000), T::from_raw(3000000000000), Rounding::exact).has_value());

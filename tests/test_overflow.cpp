...
namespace portable {
template<typename T>
constexpr bool add_overflow(T a, T b, T* out) noexcept {
...
template<typename T>
constexpr bool sub_overflow(T a, T b, T* out) noexcept {
    using U = std::make_unsigned_t<T>;
...
int main() {
    for (int a = -128; a < 128; ++a)
        for (int b = -128; b < 128; ++b) compare<std::int8_t>(static_cast<std::int8_t>(a), static_cast<std::int8_t>(b));
    ...
    const std::int64_t edges[] = {INT64_MIN,
                                  INT64_MIN + 1,
                                  -3,
                                  -1,
                                  0,
    ...
    static_assert([] {
        std::int32_t out = 0;
        return !fixedwide::detail::add_overflow(2, 3, &out) && out == 5;
    }());
    static_assert([] {
        std::int8_t out = 0;
        return fixedwide::detail::add_overflow<std::int8_t>(100, 100, &out);
    }());

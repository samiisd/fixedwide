...
static_assert(
    boundaries<fw::Fixed256<0>>("-57896044618658097711785492504343953926634992332820282019728792003956564819968",
                                "57896044618658097711785492504343953926634992332820282019728792003956564819967"));
static_assert(
    boundaries<fw::Fixed256<76>>("-5.7896044618658097711785492504343953926634992332820282019728792003956564819968",
...
consteval auto small_grid() {
    ...
    for (int raw = -128; raw <= 127; ++raw) {
        ...
        const char text[]{raw < 0 ? '-' : '+',
                          static_cast<char>('0' + mag / 100),
                          '.',
                          static_cast<char>('0' + (mag / 10) % 10),
                          static_cast<char>('0' + mag % 10),
                          '\0'};
...
void differential() {
    ...
    for (int raw = -30; raw <= 30; ++raw) {
        for (int exponent = -2; exponent <= 2; ++exponent) {
            const auto text =
                std::to_string(raw) + "e" + std::to_string(exponent - static_cast<int>(T::fractional_digits));
            compare<T>(text);
        }
    }
}

...
template<class F> void measure(const std::string& name, F loop) {
    ...
    for (unsigned repeat = 0; repeat < repetitions; ++repeat) {
        ...
        samples.push_back(std::chrono::duration<double, std::nano>(end - start).count() / static_cast<double>(iterations));
    }
    auto sorted = samples; std::sort(sorted.begin(), sorted.end());
    std::printf("%s,%zu,%u,%.6f,%.6f,%.6f,%.6f,\"", name.c_str(), iterations, repetitions,
        sorted.front(), sorted[sorted.size()/2], sorted[(sorted.size()-1)*95/100], sorted.back());

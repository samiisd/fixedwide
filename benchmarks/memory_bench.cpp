...
[[gnu::noinline]] void load_loop(const std::byte* bytes, std::size_t count) {
    for (std::size_t i = 0; i < count; ++i) {
        ...
        __asm__ __volatile__(""
                             :
                             : "r"(static_cast<std::uint64_t>(raw)), "r"(static_cast<std::uint64_t>(raw >> 64))
                             : "memory");
    }
}
...
int main(int argc, char** argv) {
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--iterations" && i + 1 < argc)
            bench::iterations = std::strtoull(argv[++i], nullptr, 10);
        else if (arg == "--repetitions" && i + 1 < argc)
            bench::repetitions = static_cast<unsigned>(std::strtoul(argv[++i], nullptr, 10));
        else
            return 2;
    }
    ...
    void* mapping =
        mmap(nullptr, static_cast<std::size_t>(page_size), PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    if (mapping == MAP_FAILED) {
        std::perror("mmap");
        return 1;
    }
    std::fprintf(stderr,
                 "compiler=%s; digits=%u; cache_line=64; anonymous resident mmap; FP128 size/alignment=%zu/%zu\n",

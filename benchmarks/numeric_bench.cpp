...
using fixedwide_bench::filter;
using fixedwide_bench::iterations;
using fixedwide_bench::measure;
using fixedwide_bench::repetitions;
...
inline void escape(float v) {
    __asm__ __volatile__("" : : "x"(v) : "memory");
}
inline void escape(double v) {
    __asm__ __volatile__("" : : "x"(v) : "memory");
}
inline void escape(std::int64_t v) {
...
inline void escape(fw::i128 v) {
    __asm__ __volatile__(""
                         ...
                         : "r"(static_cast<std::uint64_t>(v)),
                           "r"(static_cast<std::uint64_t>(static_cast<fw::u128>(v) >> 64))
                         : "memory");
}
inline void escape(fw::FP64 v) {
    escape(v.raw());
}
inline void escape(fw::FP128 v) {
    escape(v.raw());
}

template<class T>
T make(std::int64_t raw) {
    if constexpr (std::is_floating_point_v<T>)
        return static_cast<T>(static_cast<long double>(raw) / fw::scale);
    else
        return T::from_raw(raw);
}
template<class T>
T require(std::expected<T, fw::ArithmeticError> value) {
...
template<class T, Op op>
void operation(const char* type, const char* operation) {
    Inputs<T> inputs;
    ...
    measure("throughput." + suffix, [&](std::size_t count) {
        for (std::size_t i = 0; i < count; ++i) {
            ...
            if constexpr (op == Op::compare)
                escape(static_cast<std::int64_t>(inputs.a[j] < inputs.b[j]));
            else
                escape(calculate<T, op>(inputs.a[j], inputs.b[j], inputs.c[j]));
        }
    });
    if constexpr (op != Op::compare) {
...
template<class T, Op op>
void inexact_chain(const char* type, const char* name) {
    ...
    for (std::size_t i = 0; i < data_size; i += 2) {
        ...
        if constexpr (op == Op::mul_div) {
            ...
            factors[i] = make<T>(f);
            denominators[i] = make<T>(d);
            factors[i + 1] = make<T>(d);
            denominators[i + 1] = make<T>(f);
        } else {
            const auto inverse = static_cast<std::int64_t>((fw::i128{fw::scale} * fw::scale) / f);
            factors[i] = make<T>(f);
            factors[i + 1] = make<T>(inverse);
...
template<Op op>
void inexact_chain(const char* name) {
    inexact_chain<float, op>("float", name);
    inexact_chain<double, op>("double", name);
    inexact_chain<fw::FP64, op>("FP64", name);
    inexact_chain<fw::FP128, op>("FP128", name);
}
template<Op op>
...
int main(int argc, char** argv) {
    ...
    std::fprintf(stderr,
                 "compiler=%s; digits=%u; FP128 size/alignment=%zu/%zu; float/double mantissa=24/53; scalar; no FMA "
                 "contraction; seed=0x826041\n",
    ...
    floor<double>("double");
    floor<fw::FP64>("FP64");
    floor<fw::FP128>("FP128");
    operation<Op::compare>("compare");
    operation<Op::add>("add");
    operation<Op::sub>("sub");
    operation<Op::mul>("mul");
    operation<Op::div>("div");
    operation<Op::mul_div>("mul_div");
    inexact_chain<Op::mul>("mul");
    inexact_chain<Op::div>("div");
    inexact_chain<Op::mul_div>("mul_div");
}

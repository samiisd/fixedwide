...
int main() {
    ...
    const auto ex = div(parse<Money>("1.00").value(), parse<Money>("3.00").value(),
                        Rounding::exact);
    ...
    const auto failed =
        std::expected<Money, ArithmeticError>{Money::max()}
    ...
    const auto fallback = div(parse<Money>("1.00").value(), Money::from_raw(0))
                              .value_or(Money::from_raw(0));
